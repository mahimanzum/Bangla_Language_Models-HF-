...
def spawn(fn, kwargs={}, nprocs=1, join=True, daemon=False):
    r"""Spawns ``nprocs`` processes that run ``fn`` with ``kwargs``.
    ...
    Arguments:
        fn (function): Function is called as the entrypoint of the
            ...
            The function is called as ``fn(**kwargs)``
    ...
    for i in range(nprocs):
        kwargs['kwargs']['args']['local_rank'] = i
        
        error_queue = mp.SimpleQueue()
        process = mp.Process(
            target=_wrap,
            args=(fn, kwargs, error_queue),
            daemon=daemon,
        )
        process.start()
