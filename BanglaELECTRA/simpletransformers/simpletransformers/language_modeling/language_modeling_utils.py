...
def mask_tokens(inputs: torch.Tensor, tokenizer: PreTrainedTokenizer, args) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if args.dataset_class and args.model_type == "electra":
        batch = inputs
        assert len(batch) == 3
        inputs = batch[0]
        input_mask = batch[1]
    ...
    if args.model_type == "electra" and args.do_whole_word_mask:
        masked_indices = []
        ...
        for tokens in labels.tolist():
            ...
            for i, token in enumerate(tokens):
                ...
                if candidate_index_sets and tokenizer._convert_id_to_token(token).startswith('##'):
                    candidate_index_sets[-1].append(i)
                else:
                    candidate_index_sets.append([i])
            ...
            masked_indices.append([1 if i in selected_indices else 0 for i, _ in enumerate(tokens)])

        masked_indices = torch.tensor(masked_indices, dtype=torch.bool)
    ...
    if args.dataset_class and args.model_type == "electra":
        return inputs, labels, input_mask, segment_ids
