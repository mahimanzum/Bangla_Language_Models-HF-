...
class ClassificationModel:
    def __init__(
        ...
        self.tokenizer = tokenizer_class.from_pretrained(model_name, do_lower_case=self.args.do_lower_case, tokenize_chinese_chars=self.args.handle_chinese_chars, **kwargs)
    ...
    def train_model(
        ...
        **kwargs,
        ...
        elif isinstance(train_df, str):
            column_count = len(linecache.getline(train_df, 1).split('\t'))
            if column_count == 2:
                with open(train_df) as f:
                    train_examples = [
                        InputExample(i, line.split('\t')[0].strip(), None, line.split('\t')[1].strip())
                        for i, line in enumerate(f)
                    ]
            elif column_count == 3:
                with open(train_df) as f:
                    train_examples = [
                        InputExample(i, line.split('\t')[0].strip(), line.split('\t')[1].strip(), line.split('\t')[2].strip())
                        for i, line in enumerate(f)
                    ]

            else:
                raise ValueError("Invalid number of columns in traindf file.")
    ...
    def train(
        ...
        **kwargs,
        ...
        if not self.args.train_custom_parameters_only:
            if args.layerwise_lr_decay > 0 and args.model_type == "electra":
                base_lr = args.learning_rate
                n_layers = self.model.config.num_hidden_layers
                
                # embeddings, embeddings_project -> layer_depth 0
                # hidden layers -> layer_depth (hidden_layer_count + 1)
                # classification and pooler layer -> layer_depth (hidden_layer_count + 2), no layer decay i.e. base_lr

                for layer_depth in range(n_layers + 2):
                    if layer_depth == 0:
                            ...
                            [
                                {
                                    "params": [
                                        p
                                        for n, p in model.named_parameters()
                                        if n not in custom_parameter_names and not any(nd in n for nd in no_decay) and (".embeddings." in n or ".embeddings_project." in n)
                                    ],
                                    "weight_decay": args.weight_decay,
                                    "lr":  base_lr * (args.layerwise_lr_decay ** (n_layers + 2))
                                },
                                {
                                    "params": [
                                        p
                                        for n, p in model.named_parameters()
                                        if n not in custom_parameter_names and any(nd in n for nd in no_decay) and (".embeddings." in n or ".embeddings_project." in n)
                                    ],
                                    "weight_decay": 0.0,
                                    "lr":  base_lr * (args.layerwise_lr_decay ** (n_layers + 2))
                                }
                            ...
                            [
                                {
                                    "params": [
                                        p
                                        for n, p in model.named_parameters()
                                        if n not in custom_parameter_names and not any(nd in n for nd in no_decay) and (".embeddings." not in n and ".embeddings_project." not in n and '.layer.' not in n)
                                    ],
                                    "weight_decay": args.weight_decay,
                                    "lr":  base_lr
                                },
                                {
                                    "params": [
                                        p
                                        for n, p in model.named_parameters()
                                        if n not in custom_parameter_names and any(nd in n for nd in no_decay) and (".embeddings." not in n and ".embeddings_project." not in n and '.layer.' not in n)
                                    ],
                                    "weight_decay": 0.0,
                                    "lr":  base_lr
                                }
                            ...
                            [
                                {
                                    "params": [
                                        p
                                        for n, p in model.named_parameters()
                                        if n not in custom_parameter_names and not any(nd in n for nd in no_decay) and (f".layer.{layer_depth-1}." in n)
                                    ],
                                    "weight_decay": args.weight_decay,
                                    "lr":  base_lr * (args.layerwise_lr_decay ** (n_layers + 2 - layer_depth))
                                },
                                {
                                    "params": [
                                        p
                                        for n, p in model.named_parameters()
                                        if n not in custom_parameter_names and any(nd in n for nd in no_decay) and (f".layer.{layer_depth-1}." in n)
                                    ],
                                    "weight_decay": 0.0,
                                    "lr":  base_lr * (args.layerwise_lr_decay ** (n_layers + 2 - layer_depth))
                                }
                            ...
                            wandb.log(
                                {
                                    ...
                                    "lr": scheduler.get_lr()[-1] if args.layerwise_lr_decay > 0 else scheduler.get_lr()[0],
    ...
    def evaluate(
        ...
        if isinstance(eval_df, str) and self.args.lazy_loading:
            ...
            eval_examples = None
        elif isinstance(eval_df, str):
            column_count = len(linecache.getline(eval_df, 1).split('\t'))
            if column_count == 2:
                with open(eval_df) as f:
                    eval_examples = [
                        InputExample(i, line.split('\t')[0].strip(), None, line.split('\t')[1].strip())
                        for i, line in enumerate(f)
                    ]
                ...
                raise ValueError("Invalid number of columns in evaldf file.")
    ...
    def _get_inputs_dict(self, batch):
        if isinstance(batch[0], dict):
            ...
            if self.args.model_type != "distilbert":
                inputs["token_type_ids"] = batch[2] if self.args.model_type in ["bert", "xlnet", "albert", "electra"] else None
